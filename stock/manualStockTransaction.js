...
export async function main(ns) {
    ...
    const midPriceThreshold = (buyPrice + sellPrice) / 2;
    ...
    while (true) {
        ...
        if (sharesOwned > 0) {
            ...
            // Sell shares if midThreshold switch is true and price drops below midPriceThreshold
                ...
                if (totalReceived > 0) {
                    const transactionTime = new Date().toLocaleString();
                    const receiptContent = `Sold ${sharesOwned} shares of ${stockSymbol} at $${currentSellPrice.toFixed(2)} each on ${transactionTime} (Mid-Threshold Trigger).\n`;

                    await ns.write("stockReceipt.txt", receiptContent, "a");
                    ns.tprint(`Sold ${sharesOwned} shares of ${stockSymbol} (Mid-Threshold Trigger).`);
                    break; // Exit the script after selling
                }
            }
